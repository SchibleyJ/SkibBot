...
const minimax = (position, depth, alpha, beta, maximizingPlayer) => {
    ...
    if (maximizingPlayer) {
        ...
        for (let i = 0; i < 64; i++) {
            if (children[i]) {
                for (let j = 0; j < children[i].length; j++) {
                    let eval = minimax(makeMove2({ 'moveFrom': i, 'moveTo': children[i][j] }, position.board, position.whiteTurn, position.enPassantSquare, position.canCastle), depth - 1, alpha, beta, false)[0];
        ...
        for (let i = 0; i < 64; i++) {
            if (children[i]) {
                for (let j = 0; j < children[i].length; j++) {
                    let eval = minimax(makeMove2({ 'moveFrom': i, 'moveTo': children[i][j] }, position.board, position.whiteTurn, position.enPassantSquare, position.canCastle), depth - 1, alpha, beta, true)[0] 
